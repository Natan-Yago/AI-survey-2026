...
const SURVEY_NOTICE_PARAGRAPHS = [
  'תודה על השתתפותך בסקר. מטרת הסקר היא לאסוף תובנות מצטברות לגבי רמת האימוץ, הבשלות, האתגרים והעדיפויות של ארגונים ביחס לשימוש בבינה מלאכותית (AI), לרבות Generative AI ו-Agentic AI.',
  'הסקר מיועד לאיסוף מידע כללי ומצטבר. אין צורך למסור את שמך או את שם הארגון. התשובות ינותחו ויוצגו, ככל שיוצגו, באופן מצטבר ולא באופן שנועד לזהות משיב מסוים או ארגון מסוים.',
  'ההשתתפות בסקר היא וולונטרית. ניתן לבחור באפשרות "לא יודע/ת / לא ניתן למסור" או "לא יודע/ת / לא בטוח/ה", לפי העניין, כאשר אין בידך מידע מספיק או כאשר אינך רשאי/ת למסור מידע מסוים.',
  'המידע שייאסף עשוי לשמש את Deloitte ישראל לצרכים פנימיים, לניתוחים סטטיסטיים, להפקת תובנות מקצועיות, לפיתוח ידע ולפרסומים מקצועיים או שיווקיים המבוססים על נתונים מצטברים. הנתונים לא יוצגו באופן שנועד לזהות אותך או את הארגון שאליו את/ה משתייך/ת.',
  'הסקר ותוצאותיו אינם מהווים ייעוץ מקצועי, תחזית מחייבת, דירוג או הערכה פרטנית של ארגון כלשהו, ואינם תחליף לייעוץ המותאם לנסיבותיו של ארגון מסוים.',
];
          ...
          <main className="welcome-main flex-1 px-6 sm:px-10 lg:px-10 xl:px-14 pt-16 pb-12 sm:pt-20 sm:pb-16 lg:py-6 flex items-center justify-center">
            <div className="welcome-copy w-full max-w-xl">
              ...
              <details className="mb-5 border-y border-[#E5E5E5] py-4 text-sm text-[#4B5563]">
                ...
                <div className="mt-3 max-h-44 space-y-3 overflow-y-auto leading-relaxed pe-2">
                  {SURVEY_NOTICE_PARAGRAPHS.map((paragraph) => (
